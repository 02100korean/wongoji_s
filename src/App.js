...
// --- [3. 메인 앱 컴포넌트: v.11 엔진 + v.11-1 인쇄 수정 통합] ---
export default function App() {
  ...
  const allCells = useMemo(() => {
    ...
    while (i < Math.min(content.length, 3000)) {
      ...
      if ((isDigit(char) && isSimplePunct(next) && isDigit(next2)) || (isSimplePunct(char) && isDigit(next)) || (next !== "" && ((isDigit(char) && isDigit(next)) || (isAlpha(char) && isAlpha(next))))) { cells.push({ type: 'pair', content: [char, next] }); i += 2; continue; }
      ...
      else { if (qType === 'open') cells.push({ type: 'quote_open', content: char }); else if (qType === 'close') cells.push({ type: 'quote_close', content: char }); else if (isSimplePunct(char)) cells.push({ type: 'punct_alone', content: char }); else cells.push({ type: 'default', content: char }); i++; }
    }
    return cells;
  }, [content]);
      ...
      <style>{`
        ...
        @media print {
          ...
          .page-box { 
            ...
            transform: scale(min((100vw - 40mm) / 880, (100vh - 40mm) / ${gridType === '200' ? (viewMode === 'feedback' ? '850' : '600') : '1100'})) !important; 
            transform-origin: center center !important; 
          }
          /* 타입별 비율 조정 (v.11-1 요청사항 4번) */
          .type-normal { aspect-ratio: 1.414 / 1; }
          .type-feedback { aspect-ratio: 1.3 / 1; padding: 10px 0; }
                ...
                <div className="manuscript-print-root">
                  {Array.from({ length: pageCount }).map((_, p) => (
                    <div key={p} className="page-unit">
                      <div style={{ backgroundColor: 'white', padding: '40px 60px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', width: 'max-content' }} className={`page-box type-${viewMode}`}>
