...
const App = () => {
  ...
  const renderCell = (cellData, key, isLastCol) => {
    const cellStyle = { 
        ...
        borderRight: isLastCol ? `1px solid ${lineColor}` : 'none', // 마지막 칸만 오른쪽 줄 생성
        display: 'flex', 
        alignItems: 'center', 
  ...
  const Manuscript = ({ text, settings, name }) => {
    ...
    return (
      <div className="flex flex-col items-center w-full gap-12 print:gap-0 print:bg-white">
        {Array.from({ length: pageCount }).map((_, p) => (
          <div key={p} className="wongoji-paper bg-white shadow-2xl print:shadow-none flex flex-col items-center justify-start py-12 print:py-0" style={{ fontFamily: settings.fontType, ...paperStyle }}>
            ...
            {/* 원고지 격자 본체 */}
            <div className="flex flex-col print:flex-1 justify-center">
              <div className="flex flex-col" style={{ gap: rowGap }}>
                {Array.from({ length: rows }).map((_, r) => (
                  <div key={r} className="flex" style={{ borderBottom: `1px solid ${lineColor}` }}>
                    {Array.from({ length: cols }).map((_, c) => 
                      renderCell(allCells[p * gridVal + r * cols + c], `c-${p}-${r}-${c}`, c === cols - 1)
                    )}
                  </div>
                ))}
              </div>
            ...
            {/* 페이지 번호 (인쇄 시 제외 가능) */}
            <div className="w-full text-center mt-8 text-xs text-slate-300 no-print font-sans">Page {p + 1}</div>
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className={`h-screen w-full flex flex-col bg-slate-100 overflow-hidden ${gridType === '200' ? 'print-landscape' : 'print-portrait'}`}>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        <aside className="w-full lg:w-[380px] bg-white border-r no-print flex flex-col shadow-xl shrink-0">
          <div className="p-6 border-b flex flex-col gap-5">
            <div className="flex flex-col gap-3">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Settings</label>
                <div className="grid grid-cols-2 gap-2">
                    <select value={gridType} onChange={e => setGridType(e.target.value)} className="p-2.5 border rounded-xl text-sm font-bold bg-slate-50 outline-none focus:ring-2 focus:ring-red-100">
                        <option value="200">200자 (가로)</option>
                        <option value="400">400자 (세로)</option>
                    </select>
                    <select value={viewMode} onChange={e => setViewMode(e.target.value)} className="p-2.5 border rounded-xl text-sm font-bold bg-slate-50 outline-none focus:ring-2 focus:ring-red-100">
            ...
            <div className="flex flex-col gap-2">
                ...
                <input type="text" value={studentName} onChange={e => setStudentName(e.target.value)} placeholder="이름 입력" className="p-3 border rounded-xl font-bold outline-none focus:ring-2 focus:ring-red-100" />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 scroll-smooth">
            <textarea 
              ...
              className="w-full h-full min-h-[350px] p-0 border-none outline-none resize-none bg-transparent text-lg leading-loose font-serif" 
              placeholder="여기에 글을 써보세요..." 
            />
          </div>

          <div className="p-6 border-t bg-slate-50">
            <button onClick={() => window.print()} className="w-full bg-slate-900 text-white h-16 rounded-2xl font-black hover:bg-red-600 transition-all shadow-lg flex items-center justify-center gap-2 uppercase tracking-tighter">
              <Printer size={22}/> Print to PDF
            </button>
          </div>
        </aside>

        <main className="flex-1 overflow-auto bg-slate-300 p-8 lg:p-16 flex justify-center scroll-smooth">
      ...
      <style>{`
        @media print {
          ...
          main { background: white !important; padding: 0 !important; overflow: visible !important; width: 100% !important; }
          .wongoji-paper { 
            ...
            height: 100vh !important; /* 중앙 배치를 위해 세로 꽉 채움 */
          }
